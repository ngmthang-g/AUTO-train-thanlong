# CURRENT OVERRIDE — v1.5.10 Internal Confirm + Revive v0.8.7

This section supersedes any older current-design statement below that says Confirm is periodic/saved-coordinate or that donor UI RVAs must never be transplanted. Historical evidence remains below for chronology.

## Active contract
- Base is v1.5.9 rotation/route/sell architecture.
- Map Confirm has **no saved coordinate, checkbox or interval timer**.
- Revive/Đầu thai has **no saved coordinate**.
- Exactly four visible AutoFight calibration clicks remain: AUTO / ĐÁNH QUÁI / DỪNG AUTO 1 / DỪNG AUTO 2.
- Auto Sell macro coordinates are independent and remain.
- Confirm + Đầu thai execute via v0.8.7 `UIButton.HandleClickEvent()` on the existing game-window-thread Bridge.
- Exact donor fixed RVAs are permitted only for this two-action UI slice and only behind exact PE identity + required byte signatures.
- Never restore donor `remote_worker`, CreateRemoteThread or WriteProcessMemory gameplay actions.
- `MessageBox` uses a cached IL2CPP GCHandle-managed string, not repeated per-poll allocations.
- MessageBox detection is donor-exact `MainFindUI("MessageBox")` only; no `FindUI` fallback in v1.5.10.
- UIObject.instances scanning rejects disposed entries.
- Confirm requires tool-owned cross-map StartPath + authoritative MessageBox + authoritative moving=OFF.
- Revive requires a fresh authoritative `get_IsDeath=true` immediately before callback.
- Missing/ambiguous/signature-mismatch state always fails closed.
- Successful Confirm immediately enters Transition Freeze; no second action races it in the same tick.

## Validation in this workspace
- route test 15/15 PASS; rotation 8/8 PASS; static source audit PASS; lexical delimiter balance PASS.
- Windows PE compilation was not run here because Zig/Windows cross toolchain is unavailable. Runtime remains NEEDS USER TEST.

# HISTORICAL RECORDS — SUPERSEDED DESIGN NOTES BELOW

Older sections are retained only as debugging/history evidence. They may mention six click points, saved-coordinate Confirm, or earlier versions and must not override the v1.5.10 contract above.
