...
## BUG-006 — Coordinate/timer Confirm diverged from runtime-proven v0.8.7 behavior
Status: SOURCE FIX v1.5.10 / RUNTIME UNTESTED

User evidence: v0.8.7 Confirm + Đầu thai is materially smoother than v1.5.9's coordinate/timer path.

v1.5.10 resolution:
- remove periodic/saved-coordinate Confirm and coordinate Revive;
- use v0.8.7 internal UIButton callbacks;
- retain current route ownership, death cold-start and Transition Freeze;
- exact donor PE/signature gates prevent wrong-RVA calls;
- fail closed on ambiguous/missing UI.

Remaining proof required: repeated death + return-to-map cycles on the real client, including slow map transitions.
