...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "foreach($x in @('ClickSlot::AutoMenu','ClickSlot::Attack','ClickSlot::StopAuto1','ClickSlot::StopAuto2','std::array<ClickPoint, 4>','Tự Đầu thai (nội bộ v0.8.7)','HandleCrossMapConfirm','Command::ClickInternalConfirm','Command::ClickInternalRevive','HandleDeath','HandleUnderworldAutoFightGuard','HandleAutoSell','HandleTrainRecovery','SwitchToNextRotationSpot','RemoveLegacyConfirmReviveKeys')){if($controller -notmatch [regex]::Escape($x)){throw ('Missing required v1.5.10 controller token: '+$x)}};" ^
  ...
  "$proto=Get-Content -Encoding UTF8 'src/protocol.h' -Raw; foreach($x in @('ReadState = 1','ToggleRide = 2','StartPath = 3','StopPath = 4','ClickNpc = 6','ClickInternalConfirm = 7','ClickInternalRevive = 8','ValidConfirmUi','0x00010510u')){if($proto -notmatch [regex]::Escape($x)){throw ('Protocol missing v1.5.10 token: '+$x)}}; if($proto -match 'ClickAt ='){throw 'Protocol still exposes removed generic ClickAt'};" ^
