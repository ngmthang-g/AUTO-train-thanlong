...
del /q ThanLongNewCoreBridge.dll BridgeSelfTest.exe ControlSelfTest.exe ReviveSelfTest.exe ThanLongAutoTrain_NewCore_v1.2.0.exe app.res >nul 2>nul

echo [1/10] Architecture + exclusive Revive mutation audit...
...
findstr /I /N /C:"kProtocolVersion = 0x00010200u" src\common\protocol.h >nul 2>nul
...
rem Revive engine is the ONE gameplay mutation exception in v1.2.0.
...
set "REVIVE_INVOKE_COUNT=0"
for /f %%C in ('findstr /I /C:"g_api.runtime_invoke(handleClick" src\bridge\revive_action_engine.inc ^| find /C /V ""') do set "REVIVE_INVOKE_COUNT=%%C"
if not "%REVIVE_INVOKE_COUNT%"=="1" goto :revive_invoke_count_bad
...
echo Architecture audit PASS. Global gameplay mutation FALSE. Revive ONLY TRUE. Queue MAX=1. Revive direct callback count=1.
goto :audit_pass
...
echo PROTOCOL VERSION 0x00010200 NOT FOUND
goto :fail
:probe_mutation_path
...
goto :fail
:revive_invoke_count_bad
echo REVIVE ENGINE MUST CONTAIN EXACTLY ONE DIRECT runtime_invoke(handleClick); found %REVIVE_INVOKE_COUNT%
goto :fail
...
echo [5/10] Run Revive gate/ACK/map-change POST self-test...
ReviveSelfTest.exe
if errorlevel 1 (
...
echo [7/10] Bridge DLL - scanner + metadata resolver + harmless envelope + Revive ONLY action...
zig c++ -target x86_64-windows-gnu -O2 -std=c++17 -DUNICODE -D_UNICODE ^
...
echo [9/10] Controller EXE - v1.2.0 Revive Action...
zig c++ -target x86_64-windows-gnu -O2 -std=c++17 -DUNICODE -D_UNICODE -Wall -Wextra -Werror -municode -static ^
  ...
  -o ThanLongAutoTrain_NewCore_v1.2.0.exe
if errorlevel 1 goto :fail

echo [10/10] Package...
move /y ThanLongNewCoreBridge.dll dist\ThanLongNewCoreBridge.dll >nul
move /y ThanLongAutoTrain_NewCore_v1.2.0.exe dist\ThanLongAutoTrain_NewCore_v1.2.0.exe >nul
...
echo   dist\ThanLongAutoTrain_NewCore_v1.2.0.exe
echo   dist\ThanLongNewCoreBridge.dll
echo.
echo Global gameplay mutation remains FALSE.
echo Revive/UIButton.HandleClickEvent is the ONLY enabled gameplay mutation in v1.2.0.
