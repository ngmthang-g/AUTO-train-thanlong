...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$bridge=Get-Content 'src/bridge.cpp' -Raw; foreach($x in @('SendToggleRideState','StartAutoPath','StopAutoPath','get_IsDeath','get_EnableAutoF1','GetFreeBagSpace','ClickNPC')){if($bridge -notmatch [regex]::Escape($x)){throw ('Missing required route/read-only method '+$x)}}; if($bridge -match 'ReadSemanticUiPresent' -or $bridge -match 'MainFindUI'){throw 'v1.5.8 must not probe MessageBox/UI in ReadState'};" ^
