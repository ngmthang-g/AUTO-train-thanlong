...
echo [1/7] Clean Route v1.5.8 Fixed Interval Confirm Click audit...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if($controller -match 'HandleCrossMapConfirm\(a, now'){throw 'Semantic Confirm handler must not be invoked in v1.5.8'}; if($controller -match 'xác nhận sau Đầu thai'){throw 'Special revive Confirm click must be removed'}; if($controller -match '284,188' -or $controller -match '96,168' -or $controller -match 'displayX' -or $controller -match 'displayY'){throw 'NPC route coordinates must not be hard-coded'}; if($controller -match 'ExitWindowsEx' -or $controller -match 'InitiateSystemShutdown'){throw 'Timer must close tool only, never Windows'}; Write-Host 'CLEAN ROUTE v1.5.8 AUDIT PASS: periodic Confirm timer + click mutex + transition/freeze yielding + no MessageBox probe.'"
...
zig c++ -target x86_64-windows-gnu -O2 -std=c++17 -Wall -Wextra -Werror -municode -static -s ^
  ...
  -o dist\ThanLongCleanRoute_v1.5.8.exe
if errorlevel 1 exit /b 1
