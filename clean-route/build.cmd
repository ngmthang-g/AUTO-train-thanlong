...
echo [1/7] Clean Route v1.5.0 cadence + mount fallback + confirm retry audit...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "foreach($f in @('PROJECT_KNOWLEDGE.md','CHANGELOG.md')){if(!(Test-Path $f)){throw ('Missing required knowledge file '+$f)}}; if($controller -notmatch 'kTargetRecheckMs\s*=\s*180000'){throw '3-minute cadence constant changed/missing'}; if($controller -notmatch 'kMountRetryMs\s*=\s*5000'){throw '5-second mount retry changed/missing'}; if($controller -notmatch 'kFootWalkWindowMs\s*=\s*15000'){throw '15-second foot fallback changed/missing'};" ^
  "if($controller -match '284,188' -or $controller -match '96,168' -or $controller -match 'displayX' -or $controller -match 'displayY'){throw 'NPC route coordinates must not be hard-coded'}; Write-Host 'CLEAN ROUTE v1.5.0 AUDIT PASS: user NPC coords retained; 3-minute train coordinate cadence, mount retry/walk fallback, confirm retry, and knowledge files are present.'"
...
zig c++ -target x86_64-windows-gnu -O2 -std=c++17 -Wall -Wextra -Werror -municode -static -s ^
  ...
  -o dist\ThanLongCleanRoute_v1.5.0.exe
if errorlevel 1 exit /b 1
